...
// Net between currentUser and friendId across a set of expenses (projects multi-person)
function netBetween(exps: RichExpense[], myId: string, friendId: string): number {
  ...
  for (const exp of exps) {
    ...
    // Someone else paid — no direct debt between me and friend
  }
  return net;
}

// Full settlement engine for a group of expenses
...
const EXTRA_CSS = `
  ...
  .sl-add-member-wrap { position: relative; }
  .sl-add-member-results { position: absolute; top: calc(100% + 4px); left: 0; right: 0; z-index: 50; background: var(--surface); border: 1px solid var(--border); border-radius: 10px; box-shadow: 0 8px 24px rgba(0,0,0,.1); overflow: hidden; }
...
type Selected = { type: "friend"; userId: string } | { type: "group"; groupId: string } | null;
...
export default function DashboardPage() {
  ...
  // Friends = anyone else who appears in a direct expense with me
  ...
  function FriendPanel({ f }: { f: FriendSummary }) {
    ...
    async function handleSettle() {
      if (!hasBalance || settling) return;
      setSettling(true);
      try {
        // Record a settlement expense: the debtor paid, split FULL to themselves
        const debtorId  = f.netOwed > 0 ? f.userId : myId;
        const creditorId = f.netOwed > 0 ? myId : f.userId;
        const amt = Math.abs(f.netOwed);
        await createExpense({
          title: `Settlement: ${f.displayName}`,
          amount: amt,
          splitMethod: "FULL",
          paidBy: debtorId,
          participantUserIds: [debtorId, creditorId],
          participantShareCounts: [100, 0],
        });
        setSettled(true);
    ...
    return (
      <>
        ...
        {settled && (
          <div className="sl-settled-banner">
            ✅ Marked as settled! Refreshing…
          </div>
        )}
        ...
        <Link to={`/friend/${f.userId}`} className="sl-see-details">
          <span>See all expenses with {f.displayName}</span>
  ...
  function GroupPanel({ gs }: { gs: GroupSummary }) {
    ...
    const addCandidates = memberSearch
      ? [...userMap.entries()]
          .filter(([uid, info]) => !memberIds.has(uid) && (info.email.toLowerCase().includes(memberSearch.toLowerCase()) || info.displayName.toLowerCase().includes(memberSearch.toLowerCase())))
          .slice(0, 5)
      : [];
    ...
    return (
      <>
        ...
        {showMembers && (
          <div className="sl-section-card">
            ...
            <div className="sl-section-card-body">
              ...
              {/* add member search */}
              <div className="sl-add-member-wrap" style={{ marginTop: 8 }}>
                <input
                  ...
                  onFocus={() => !membersLoaded && loadMembers()}
                />
                {addCandidates.length > 0 && (
                  <div className="sl-add-member-results">
                    {addCandidates.map(([uid, info]) => (
                      <div key={uid} className="sl-add-result" onClick={() => !addingId && handleAdd(uid)}>
                        <div className="sl-member-av" style={{ background: "var(--accent-bg)", color: "var(--accent)", width: 28, height: 28, borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", fontSize: ".68rem", fontWeight: 700, flexShrink: 0 }}>
                          {initials(info.displayName)}
                        </div>
                        <div>
                          <div style={{ fontSize: ".84rem", fontWeight: 500 }}>{info.displayName}</div>
                          <div style={{ fontSize: ".72rem", color: "var(--muted)" }}>{info.email}</div>
                        </div>
        ...
        <Link to={`/group/${group.id}`} className="sl-see-details">
          <span>See all expenses in {group.name}</span>
          <span className="sl-see-details-arrow">→</span>
        </Link>
      </>
    );
